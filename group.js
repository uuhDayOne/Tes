...
module.exports = welcome = async (zaki, anu) => {
		...
		try {
            ...
            zaki.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika Anda Menyewa Bot Srgera Lapor Owner Agar Dimasukkan Ke List Sewa, Jika Tidak Maka Bot Otomatis Akan Out Dalam 20 Menit', 'conversation')
            }
             if (anu.action == 'add' && !mem.includes(zaki.user.jid)) {
      ...
      if (anu.action == "remove" && !mem.includes(zaki.user.jid)) {
        ...
        zaki.sendMessage(anu.jid, { contentText: '\`\`\`AKHIRNYA SI BEBAN OUT👋\`\`\`', footerText: '✗✗✗ ɪ ᴀᴍ ᴢᴀᴋʙᴏᴛᴢ', buttons: [{ buttonId: `!bay`, buttonText: { displayText: 'AKHIRNYA SI BEBAN OUT👋' }, type: 1 }], headerType: 6, locationMessage: { degreesLatitude: 0, degreesLongitude: 0, jpegThumbnail: buff2 }}, 'buttonsMessage')    
        }
		} catch (e) {
